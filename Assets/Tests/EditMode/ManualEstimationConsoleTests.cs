...
public sealed class ManualEstimationConsoleTests
{
    ...
    public void ManualCycleSourceHoldsTheTwelveEstimationProblemsWithoutPractice()
    {
        ...
        Assert.That(
            ...
            Is.False);
        Assert.That(
            catalog.problems.Select(problem => StudyRouteIdentity.GetRouteCode(problem.id)).Distinct().Count(),
            Is.EqualTo(12),
            "The console names every problem by its code, so the codes cannot collide.");
    }

    /// <summary>
    /// Manual estimation cycles write recordings but no session manifest, and manual-run recovery
    /// reports any manifest-less directory under its own root as unresolved data - which blocks the
    /// console on the next launch. That is why the two roots have to stay separate.
    /// </summary>
    [Test]
    public void ManualEstimationRecordingsStayOutOfTheManualRunRecoveryRoot()
    {
        Assert.That(
            ManualEstimationPolicy.RecordingRootName,
            Is.Not.EqualTo(ManualEstimationPolicy.ManualRunRootName));

        string manualRoot = Path.Combine(
            Path.GetTempPath(),
            "vhard-manual-" + Guid.NewGuid().ToString("N"),
            ManualEstimationPolicy.ManualRunRootName);
        Directory.CreateDirectory(Path.Combine(manualRoot, "20260814_120000_000"));
        try
        {
            Assert.That(
            ...
            Assert.That(
                diagnostic,
                Is.Not.Empty,
                "A manifest-less directory under the run root blocks startup recovery.");
        }
        finally
        {
            Directory.Delete(Path.GetDirectoryName(manualRoot), true);
        }
    }
